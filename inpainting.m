...
tic
[L, S] = RPCA(X, lambda, 1.0, 1e-5, 1000);
toc
...
% show the results
figure;
subplot(2,2,1), imshow(img_corrupted), title('Img')
subplot(2,2,2), imshow(img_final), title('Recovered Img')
subplot(2,2,3), imshow(img_reconstructed), title('L')
subplot(2,2,4), imshow(img_noise), title('S')
